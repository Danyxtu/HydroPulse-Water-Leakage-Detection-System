...
export const waterService = {
  ...
  getZones: async (): Promise<ApiResponse<Zone[]>> => {
    try {
      ...
      return { success: false, data: [], message: 'Failed to connect to backend' };
    }
  },
  ...
  applyLocalUpdate: (id: string, updates: Partial<Zone>, currentZones: Zone[]): Zone[] => {
  ...
  startDetectionScan: async (): Promise<ApiResponse<{ success: boolean; results: any }>> => {
    try {
      return await apiClient.post('/detection/scan', {});
    } catch (e) {
      return { success: false, data: { success: false, results: null }, message: 'Failed to start scan' };
    }
  },
